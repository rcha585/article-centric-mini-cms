...
router.get("/:aid/comments", async (req, res) => {
  ...
  const comments = await db.all(
    "SELECT c.id AS comment_id, c.content, c.created_at, u.username, u.id AS user_id, u.avatar_id, a.avatar_path, (SELECT COUNT(*) FROM subscriptions WHERE subscribed_user_id = u.id) AS total_subscriptions FROM comments AS c, users AS u, avatars AS a WHERE c.user_id = u.id AND u.avatar_id = a.id AND c.article_id = ? ORDER BY c.created_at ASC",
    req.params.aid
  );
